...
ed(term goal, term *frame)
{
	...
	char buf[128], *editor, *temp_file = tmpnam(NULL);
	...
	if (old_defn)
	{
		...
		{
			output = old_output;
			fail("Couln't open tmp file for editor");
		}
		list_proc(old_defn);
		fclose(output);
		output = old_output;
...
send_to(char *cmd)
{
	...
	input = fdopen(parent_i, "r");
	output = fdopen(parent_o, "w");
}
...
p_select(term goal, term *frame)
{
	...
	int interval = IVAL(check_arg(2, goa, frame, INT, IN));
	...
	if (select(n+1, &readfs, &writefds, &exceptfds, tp) == 0)
		return(FALSE);
	...
	for(i = 0; i < n; i++)
	{
		...
		if (FD_ISSET(fd, &readfs) ||  FD_ISSET(fd, &writefds))
